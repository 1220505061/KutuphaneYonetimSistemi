...
public class BookDAO {
    ...
    public void addToWaitlist(int bookId, int memberId) {
        try (java.sql.Connection c = database.DatabaseConnection.getInstance().getConnection();
             java.sql.PreparedStatement p = c.prepareStatement("INSERT INTO waitlist (book_id, member_id) VALUES (?, ?)")) {
            p.setInt(1, bookId);
            p.setInt(2, memberId);
            p.executeUpdate();
        } catch (Exception e) { e.printStackTrace(); }
    ...
    public boolean isWaiting(int bookId, int memberId) {
        try (java.sql.Connection c = database.DatabaseConnection.getInstance().getConnection();
             java.sql.PreparedStatement p = c.prepareStatement("SELECT * FROM waitlist WHERE book_id=? AND member_id=?")) {
            p.setInt(1, bookId); p.setInt(2, memberId);
            return p.executeQuery().next();
        } catch (Exception e) { return false; }
    }

    public java.util.List<Integer> getWaitingMembers(int bookId) {
        java.util.List<Integer> list = new java.util.ArrayList<>();
        try (java.sql.Connection c = database.DatabaseConnection.getInstance().getConnection();
             java.sql.PreparedStatement p = c.prepareStatement("SELECT member_id FROM waitlist WHERE book_id=?")) {
            p.setInt(1, bookId);
            java.sql.ResultSet rs = p.executeQuery();
            while(rs.next()) list.add(rs.getInt("member_id"));
        } catch (Exception e) { e.printStackTrace(); }
        return list;
    }

    public void clearWaitlist(int bookId) {
        try (java.sql.Connection c = database.DatabaseConnection.getInstance().getConnection();
             java.sql.PreparedStatement p = c.prepareStatement("DELETE FROM waitlist WHERE book_id=?")) {
            p.setInt(1, bookId);
            p.executeUpdate();
        } catch (Exception e) { e.printStackTrace(); }
    }

    // BİLDİRİM GÖNDERME (Notification)
    public void addNotification(int memberId, String message) {
        try (java.sql.Connection c = database.DatabaseConnection.getInstance().getConnection();
             java.sql.PreparedStatement p = c.prepareStatement("INSERT INTO notifications (member_id, message) VALUES (?, ?)")) {
            p.setInt(1, memberId);
